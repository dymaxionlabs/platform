...
def run_subprocess(cmd, env=None, cwd=None):
    ...
    subprocess.run(cmd, env=env, cwd=cwd, shell=True, check=True)
...
def gsutilCopy(src, dst, recursive=True):
    ...
    run_subprocess(
        "{sdk_bin_path}/gsutil -m cp {r} {src} '{dst}'".format(
            sdk_bin_path=settings.GOOGLE_SDK_BIN_PATH, r=r, src=" ".join(src), dst=dst
        )
    )
