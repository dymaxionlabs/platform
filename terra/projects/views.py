...
class RelatedProjectAPIView(APIView):
    def get_project(self):
        ...
        if project_param:
            ...
            if not project:
                raise ValidationError({"project": "Project invalid or not found"})
...
class ProjectInvitationTokenViewSet(mixins.RetrieveModelMixin, viewsets.GenericViewSet):
...
class ConfirmProjectInvitationView(APIView):
    def post(self, request, key):
        ...
        return Response(
            {"detail": _("ok"), "project": invitation.project.uuid},
        ...
        if settings.MAILCHIMP_APIKEY is not None:
            ...
            try:
                client.lists.members.create(
                    list_id,
                    {"email_address": email, "status": "subscribed", "tags": [landing]},
                )
                return Response(
...
class MapViewSet(ProjectRelatedModelListMixin, viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        if project_uuid is None:
            res = (
                res | self.queryset.filter(extra_fields__public=True).distinct()
            ).distinct()

        return res.all()


class LayerViewSet(ProjectRelatedModelListMixin, viewsets.ReadOnlyModelViewSet):
...
class UserAPIKeyViewSet(generics.ListCreateAPIView, mixins.UpdateModelMixin):
    ...
    def create(self, request):
        ...
        api_key, key = UserAPIKey.objects.create_key(
            name=request.data["name"], user=request.user, project=project
        )
